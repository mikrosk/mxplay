...
static BOOL		playlistOpened = TRUE;
...
void PanelPlay( void )
{
	...
	if( AudioPluginCheckHeader() == TRUE )
	{
	...
	else if( g_currPath == NULL || g_currName == NULL )
	{
		/* no file played yet */
		if( g_emptyPlayList == FALSE )
		{
			PlayListPlayFromFirstFile();
		}
		else
		{
			/* no file in playlist */
...
void LoadAndPlay( void )
{
	...
	if( g_currPath != NULL && g_currName != NULL )	/* only if some file in playlist */
	{
		...
		{
			AudioPluginCheckFunctions( g_pCurrAudioPlugin );
