; XM player + Pulsar library
; made by Sqward / Mystic Bytes
; additional cosmetic touches by MiKRO / Mystic Bytes
; version 0.20
...
; ------------------------------------------------------
		...
		include	pulsar\std_02.s

; ------------------------------------------------------
		SECTION TEXT
; ------------------------------------------------------

; Base mxPlay functions

; Register

xm_register_module:
		movea.l	xm_header+MXP_PLUGIN_PARAMETER,a0
		move.l	(a0)+,xm_buffer
		move.l	(a0),xm_length
		
		movea.l	xm_buffer,a0
		lea	xm_magic_id,a1
		moveq	#17-1,d7
.loop1:		cmp.b	(a0)+,(a1)+
		dbne	d7,.loop1
...
.ok:		lea	xm_module_name,a1
		moveq	#20-1,d0
.loop2:		move.b	(a0)+,(a1)+
		dbra	d0,.loop2
		addq.l	#1,a0				; some strange character
		clr.b	(a1)				; terminate it (for sure)
		
		lea	xm_tracker_name,a1
		moveq	#20-1,d0
.loop3:		move.b	(a0)+,(a1)+
		dbra	d0,.loop3
		clr.b	(a1)				; terminate it (for sure)
		
		lea	xm_module_version,a1
		move.b	#'1',(a1)+
		move.b	#'.',(a1)+
		move.b	1(a0),d0
		add.b	#$33,d0
		...
		movei.l	(a0)+,d0			; header size
		
		movei.w	(a0)+,d0			; song length
		move.w	d0,Song_len
		
		movei.w	(a0)+,d0			; restart position
		move.w	d0,XM_restart_song
		
		movei.w	(a0)+,d0			; number of channels
		move.w	d0,channel_Nb
		
		movei.w	(a0)+,d0			; number of patterns
		move.w	d0,pattern_Nb
		
		movei.w	(a0)+,d0			; number of instruments
		move.w	d0,Instrument_Nb
		
		clr.b	XM_Amiga
		movei.w	(a0)+,d0			; type of table used
		
		btst	#0,d0				; test frequency table used
		beq.b	.amiga_table
		
.linear_table:	addq.b	#1,XM_Amiga
		move.w	#$1e40,XM_max_period
		move.w	#$c0,XM_min_period
		bra.b	.skip

.amiga_table:	clr.b	XM_Amiga
		move.w	#$1ac0,XM_max_period
		move.w	#$f,XM_min_period

.skip:		movei.w	(a0)+,d0			; default tempo
		move.w	d0,default_tempo

		movei.w	(a0)+,d0			; default BPM
		move.w	d0,default_BPM
		
		move.l	a0,xm_header_end_buffer		; end of header
...
xm_init:	move.w	#3,-(sp)			; mxalloc() - prefer ttram
		move.l	#96*256*2,-(sp)			;
		move.w	#$44,-(sp)			;
		trap	#1				;
		addq.l	#8,sp				;
		...
		moveq	#MXP_ERROR,d0
		rts
.ok:		move.l	d0,XM_Pertab
		moveq	#MXP_OK,d0
		rts

; Deinit()

xm_deinit:	pea	XM_Pertab			; Mfree()
		move.w	#$49,-(sp)			;
		trap	#1				;
		addq.l	#6,sp				;
...
xm_set:		movea.l	xm_header_end_buffer,a0		; patched to the begin of binary
		jsr	XM_init_module
				
		moveq	#2,d0				;play at 32Khz
		jsr	XM_init_sound
		jsr	XM_Play
...
xm_unset:	jsr	XM_deinit_sound

		moveq	#MXP_OK,d0
		rts

; Services for mxPlay structures

xm_get_module_name:
		move.l	#xm_module_name,xm_header+MXP_PLUGIN_PARAMETER
		rts
		
xm_get_tracker:
		move.l	#xm_tracker_name,xm_header+MXP_PLUGIN_PARAMETER
		rts

xm_get_module_channels:
		clr.l	d0
		move.w	channel_Nb,d0
		move.l	d0,xm_header+MXP_PLUGIN_PARAMETER
		rts
		
xm_get_module_version:
		move.l	#xm_module_version,xm_header+MXP_PLUGIN_PARAMETER
		rts
...
XM_fill_skp_1:	move.w	d0,(a0)+
		...
		lea	XM_skip_tab,a0			; ok, now create lookup for "packed enties"
		move.w	#%10000000,d2
		move.w	#32-1,d7
...
; ננננננננננננננננננננננננננננננננננננננננננננננננננננננ
;
...
;נננננננננננננננננננננננננננננננננננננננננננננננננננננננ
;
...
XM_PLAY_INT:	move.b	(a0)+,d0			;depack channel
		bmi.b	unpack_note
...
;נננננננננננננננננננננננננננננננננננננננננננננננננננננננ
;
;This little routine process all the stuff that have to be
...
xm_magic_id:	dc.b	"Extended Module: "		; magic for the right XM identification
		even
...
xm_module_name:	ds.b	20+1				; place for 20 charactes + 0
		even
xm_tracker_name:
		ds.b	20+1				; place for 20 charactes + 0
		even
xm_module_version:
		ds.b	4				; "1.X" + 0
		even
