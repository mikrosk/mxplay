...
receive_loop:	wait_receive
		movep	x:>>HRX,x:(r0)+
		jset	#>3,X:<<M_HSR,receive_loop
...
mix_16bit:	move	x:output_ptr,r6
...
around:
	...
	;------------------------------------
		...
		rti
;---GET VOLUME------------------------------------------------------------------
;	y0/y1 - left and right volume
;

get_volume:	move	a,y:temp1
		move	x:(r0)+,x1
		nop
		...
		move	x:vol_reci,y0
		mpy	y0,x1,a		;volumized
		move	a,y1
		mpy	x0,y1,b	#$7fffff,x1
		move	b,x0
