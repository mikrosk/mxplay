...
P_MIX_1:
   ...
   ********************************************************************
   *	   Mix 8bit with loops, bouth normal and ping-pong,           *
...
spl_no_change	moveq	#0,d1
	...
	******************************************
		...
		swap.w	d1
		move.l	a2,a3
		add.w	d1,a3		;will point 2 next frame start adress
...
mix_normal_loop:
	...
	******************************************
		...
		move.l	spl_now_playing(a0),a2	;calc current end mix pointer
		move.l	d0,d1
		move.w	bufer_len(pc),d3
		ext.l	d3
		mulu.l	d3,d1
		...
		add.l	spl_loop_off(a0),d5	;a4=spl loop ptr
		add.l	spl_loop_len(a0),d5	;a4=spl loop end ptr
		move.l	d5,d6
		
		sub.l	a3,d5
		bpl.b	mix_normal_loop_enter_2
		neg.l	d5			;d5=-d5
		sub.l	d5,d3			;mix d3 samples before loop end ptr 
						;mix d5 after loop ptr -> loop restarted
		cmp.l	a2,d6			;have we reached spl loop end?
		bhi.b	mix_no_loop_reinitialize

		clr.l	d3
mix_no_loop_reinitialize:
		st	spl_loop_flag1(a0)
		bra.b	spl_mix_full_buffer2

mix_normal_loop_enter_2:
		moveq	#0,d5
		clr.b	spl_loop_flag1(a0)
		move.l	a3,spl_now_playing(a0)

spl_mix_full_buffer2
		swap.w	d0
		
		move.l	d3,d7		;"before loop end ptr"
		add.l	d5,d7		;plus "after loop ptr"! (when no loop or loop not expired then d4 always equals to 0)
		put_host	d7
		subq.w	#1,d3
		bmi.b	mix_after_samples
	
		add.l	d0,d2

mix_samples_inner_2:	move.b	(a2,d2.w),(a1)	;dsp send 8bit
		addx.l	d0,d2
		dbf	d3,mix_samples_inner_2

mix_after_samples
		tst.b	spl_loop_flag1(a0)
		beq.b	spl_sample_not_expired2
		subq.w	#1,d5
		bmi.b	spl_sample_not_expired2

		move.l	spl_ptr(a0),a2
		add.l	spl_loop_off(a0),a2	;a2=spl loop ptr
		clr.w	d2

		add.l	d0,d2

mix_samples_inner_2_in_loop:	move.b	(a2,d2.w),(a1)	;dsp send 8bit
		addx.l	d0,d2
		dbf	d5,mix_samples_inner_2_in_loop

		add.w	d2,a2
		move.l	a2,spl_now_playing(a0)

spl_sample_not_expired2:
