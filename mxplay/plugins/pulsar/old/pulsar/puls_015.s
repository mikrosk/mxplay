...
PL_clr_voices:	move.l	d0,(a0)+
		...
		move.w	#4096*2-1,d7
		moveq	#0,d0
mk_3_loop:	move.l	d0,d1
		divu.w	#3,d1
		cmp.w	#1,d1
		bgt.b	fuck_3_div
		moveq	#1,d1
fuck_3_div	move.w	d1,(a0)+
		addq.l	#1,d0
		dbf	d7,mk_3_loop
...
spl_mix_no_void:
	;
		...
		cmp.l	#$10000,d0	;if < $1.2 then no 030 resampling!
		ble	mix_8_wo_res
	;
...
mix_8_wo_res:	
		...
		lea	$fffffa205.w,a1	;host adr -> a1
...
msi8wores_e:	dbf	d1,msi8wores
		...
		moveq	#16-1,d7
drdspp_5	clr.l	-1(a1)	;dsp send 8bit
		nop
		nop
		dbf	d7,drdspp_5
		rts
...
mp:
		...
		;addq.w	#1,d1	
		bra.b	msppp
msi2ppp:	move.b	-(a2),(a1)	;dsp send 8bit
		move.b	-(a2),1(a1)
		move.b	-(a2),(a4)
...
mlfppp:
		...
		move.l	a2,spl_now_playing(a0)
		moveq	#16-1,d7
drdspp_3	move.b	-(a2),(a1)	;dsp send 8bit
		move.b	-(a2),1(a1)
		move.b	-(a2),(a4)
		dbf	d7,drdspp_3
ME2pp
