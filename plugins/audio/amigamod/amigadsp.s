...
; "small" memory violation bugfix by MiKRO / Mystic Bytes (30.12.2005)
;
; Set TAB size = 5 (don't ask me why, I've just observed it ;-)

		Section	TEXT

CPU_Time		equ		0		; Display CPU-Time taken by the Replay
DSP_Buffer	equ		2048		; DSP Sample Buffer Size for 49.17 KHz
...
***********	Jouer/arrˆter le module, Position			***********
...
; This is a real mess. This routine copies 672 bytes just bellow each sample
; into the original module! I have no idea what this 672 number means I just
; replaced copying into original module into newly allocated buffer and killed
; original 20 KB (area for *ALL* samples!) long buffer "MGTK_WorkSpace_Adr" in bss.

MGTK_Total_Length
		moveq.l	#0,d3				; Longueur
		move.w	Amiga_Length(a2),d3		; du sample
		add.l	d3,d3				; * 2 car stock‚ en words
		add.l	#672,d3				; magic constant ;-)
		...
		tst.l	d0
		beq.w	MGTK_Init_Samples_Error
		
		move.l	d0,MGTK_WorkSpace_Adr
		
		movea.l	a1,a3				; begin of samples
		movea.l	d0,a4				; free area (two times bigger than size of all samples)
MGTK_Move_Samples
		move.b	(a3)+,(a4)+			; copy samples
		subq.b	#1,d1
		bne.b	MGTK_Move_Samples
		
		movea.l	a1,a4				; this is our "temporary area" (in fact, original mod!)
		movea.l	d0,a1				; this is our new start of samples
...
MGTK_Copy_Sample_Loop
		...
		move.w	#672/4-1,d0
MGTK_Clear_End_Sample_Loop					; et met du vide aprŠs
...
MGTK_Copy_First_Loop
		...
		dbra		d0,MGTK_Copy_First_Loop		; une premiŠre fois
...
MGTK_Make_Loop_Buffer
		...
		sub.w	d5,d3
		sub.w	d4,d3				; Saute la partie aprŠs
...
; Envoie le Nombre de Voies pour le mixage (sans les 4 voies FX)
...
MGTK_Replay_Get
		...
		move.l	Voice_Next(a5),d0
		add.l	d0,MGTK_Voices_Ptr
...
MGTK_Panoramics
; les Voies FX L-R-L-R
		dc.l		+1*Voice_Size,+1*Voice_Size,+1*Voice_Size,+1*Voice_Size
; les Voies Normales L-R-R-L
; 1 + 1 -> 2 + 2 -> 4 - 1 -> 3 + 2 -> 5
		Rept		8
...
MGTK_Old_Module			ds.b		1
						...
						Even
MGTK_Fx_Voices				ds.b		4*Voice_Size
